    ...
    def render(self):
        if self.render_mode is None:
            logger.warn(
                ...
                "You can specify render_mode at initialization, "
...
def run_demo(episodes: int, max_steps: int, render_mode: Optional[str], seed: Optional[int]) -> None:
    ...
    for ep in range(episodes):
        ...
        for step_idx in range(max_steps):
            # A tiny hand-coded policy that is good enough for a demo run.
            action = env.action_space.sample()
            # action = int((obs[2] + 0.25 * obs[3]) > 0.0)
            obs, reward, terminated, truncated, _ = env.step(action)
            total_reward += reward
            if terminated or truncated:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help="Render mode. Use 'human' to open a pygame window.",
    )
